...
rule download_bash:
    input:
        script=lambda wc: f"{workflow.basedir}/{cfg.get_from_dataset(wc.dataset, key='download_script')}"
    output:
...
rule process_to_adata:
    ...
    params:
        ...
        script=lambda wc: f"{workflow.basedir}/{cfg.get_from_dataset(wc.dataset, key='process_script')}"
    wildcard_constraints:
...
rule process_to_adata_multi:
    ...
    params:
        script=lambda wildcards: f"{workflow.basedir}/{cfg.get_from_dataset(wildcards.dataset, key='process_script')}",
        input_dir=lambda wildcards: f"data/raw_data/{wildcards.dataset}",
